...
namespace WorkerApp.Workers
{
    public static partial class BillWorker
    {
        ...
        {
            var totalFee = bill.TotalAmountRequested * 0.005m;
            ...
            Db.FeesCollection.InsertOne(new ServerLib.Models.FeeData
            {
                Date = DateTime.UtcNow,
                Currency = bill.Currency,
                Type = ServerLib.Models.FeeType.Payment,
                Units = bill.TotalFees[0] - totalFee,
            }) ;
        ...
        {
            var feep = bill.MethodType == MethodType.CreditCard ? 0.014m : 0.008m;
            var totalFee = bill.TotalAmountRequested*feep + 0.25m;
        ...
        private static void Action(this JobPromoteActionData action, IClientSessionHandle s)
        {
            if (action.IsSearch)
                ...
                Db.JobPostsCollection.UpdateOne(s, x => x.Id == action.PostId, new UpdateDefinitionBuilder<JobPost>()
                .Set(x => x.Promoted, true));
        }
