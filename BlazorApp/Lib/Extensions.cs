...
namespace BlazorApp.Lib
{
    public static class Extensions
    {
        ...
        public static async Task<HttpResponseMessage> UploadFile(this IFileReference file, string format = null)
        {
            var info = await file.ReadFileInfoAsync();
            using var form = new MultipartFormDataContent();
            using var fcontent = new StreamContent(await file.OpenReadAsync());
            using var token = new StringContent(Session.Token.ToJson());
            using var fmt = new StringContent(format ?? info.Type);
            
            form.Add(fcontent, "file", info.Name);
            form.Add(fmt, "format");
            form.Add(token, "token");

            var resp = await Ez.HttpClient.PostAsync(Ez.ApiUrl + "media/uploadfile", form);
            return resp;
        }
