...
namespace OTaff.Controllers
{
    ...
    public class PostController : ControllerBase
    {
            ...
            [FromForm] string jpost)
        {
            ...
            post.Images = post.Images.Take(sub.Limits.ImageCount).ToList();
            ...
            foreach (var i in post.Images)
                Db.UserMediaCollection.UpdateOneAsync(x => x.FullUrl == i, 
                    new UpdateDefinitionBuilder<UserMedia>().Set(x => x.ExpiresOn, DateTime.UtcNow + TimeSpan.FromDays(35)));

            _ = Db.DraftJobPosts.DeleteOneAsync(x => x.UserId == jwt.UserId);
