    ...
    public class BillingController : ControllerBase
    {
        ...
        public ActionResult<string> ConfirmBill([FromForm] string token, [FromForm] string billId)
        {
            try
            {
                ...
                if (bill.Paid) 
                {
                    _ = MailController.SendNetRelayMail(jwt.Email, "Bill has been paid", $"<a href='{Ez.ClientUrl}/bill/{bill.Id}/details'>View bill</a>");
                ...
                else
                {
                    var upd = new UpdateDefinitionBuilder<UserBill>()
                        ...
                        .Set(x => x.ShouldAct, false);
                    Db.UserBillsCollection.UpdateOne(x => x.Id == billId, upd);
                    _ = MailController.SendNetRelayMail(jwt.Email, "Bill coud not be paid", $"<a href='{Ez.ClientUrl}/bill/{bill.Id}/details'>View bill</a>");
