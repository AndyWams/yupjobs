...
namespace OTaff.Controllers
{
    ...
    public class ReviewsController : ControllerBase
    {
            ...
            [FromForm] string profileId)
        {
            ...
            _ = Task.Run(() =>
            {
                var id = review.Type is ReviewType.Employee ? job.EmployerId : job.EmployeeId;
                var user = Db.UsersCollection.First(x => x.Id == id);
                _ = MailController.SendNetRelayMail(user.Email, "You have been reviewed", $"A review of your profile has been submitted <br/> {System.Web.HttpUtility.HtmlEncode(review.ReviewContent)}");
            });
