    ...
    public class PayoutsController : ControllerBase
    {
            ...
            [FromForm] string token)
        {
            try
            {
                ...
                _ = MailController.SendNetRelayMail(user.Email, "Payouts account created", "Your payouts account has been created, please complete your profile on the <a href='dashboard.stripe.com'>Stripe Dashboard</a>");
        ...
        public async Task<ActionResult<string>> Transfer([FromForm] string token, [FromForm] string walletId, [FromForm] decimal amount = 0)
        {
            try
            {
                ...
                s.CommitTransaction();
                _ = MailController.SendNetRelayMail(jwt.Email, "Payout initiated", $"A payout of {Math.Round(action.Amount / 100, 2, MidpointRounding.ToNegativeInfinity)} <br/><a href='dashboard.stripe.com'>Go to stripe dashboard</a>");
